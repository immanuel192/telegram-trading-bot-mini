# order-caching Specification

## Purpose
Defines the in-memory order caching system for trade-manager to support high-frequency order processing and ensure eventual consistency with the database through reactive updates and periodic synchronization.
...
- **THEN** it SHALL maintain internal mappings:
  - `Map<OrderId, { orderId: string, symbol: string, side: string, messageId: number, channelId: string, lotSize: number, lotSizeRemaining: number, takeProfits: { price: number, isUsed?: boolean }[], isTpMonitoringAvailable?: boolean }>`
  - `Map<AccountId, Set<OrderId>>` for account-level indexing
  - `Map<Symbol, Set<OrderId>>` for symbol-level indexing
...
The cache SHALL support efficient lookup of orders by symbol to handle high-frequency price updates.

#### Scenario: Find Orders by Symbol
- **WHEN** querying for a symbol (e.g., XAUUSD)
- **THEN** it SHALL return all active cached orders for that symbol across all accounts using the internal symbol index.

### Requirement: Account Monitoring State Sync
The cache SHALL automatically determine if monitoring is enabled for an order based on the account's configuration.

#### Scenario: Populate Monitoring Availability
- **WHEN** adding an order to the cache
- **THEN** it SHALL fetch the account configuration, check `enableTpMonitoring`, and set `isTpMonitoringAvailable` on the cached order.
...
- **THEN** it SHALL provide the following methods:
  - `addOrder(...)`: Adds a new order to the cache, populating all fields and indices.
  - `updateOrder(orderId: string, data: Partial<CachedOrder>)`: Updates an existing order's fields (lot size, TP tiers, etc.).
  - `removeOrder(orderId: string)`: Removes an order from the cache and cleans up all related indices (account, symbol).
  - `getOrdersBySymbol(symbol: string)`: Retrieves all orders for a symbol.
